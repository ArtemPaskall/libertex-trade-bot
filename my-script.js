...
class ZeroMd extends HTMLElement {
  ...
  buildStyles() {
    ...
    const html = `<div class="markdown-styles"><style>${
      this.config.hostCss
    }</style>${get('template[data-merge="prepend"]')}${
  ...
  async buildMd(opts = {}) {
    const src = async () => {
      ...
      if (resp.ok) {
        ...
        //   ...md.matchAll(/<codalized( main="(js|ts|py|java|cs)")?\/>/gim),
        // ]
        ...
        //   const codalized =
        //     /<((js|ts|py|java|cs)(-js|-ts|-py|-java|-cs)*)>([\s\S]*?)<\/\1>/gim
        ...
        //   /<!--(js|ts|py|java|cs)(\W)(.*?)\2(.*?)\2-->/gim
        // ;[...md.matchAll(translationPerCodeOption)].forEach(
        //   ([match, perCode, __, from, to]) => {
        ...
        //   ...md.matchAll(/<localized( main="(uk|ru|en)")?\/>/gim),
        // ]
        ...
        // ;[...md.matchAll(translationPerLangOption)].forEach(
        //   ([match, perLang, __, from, to]) => {
        ...
        // const [, poetryBoldStart, poetryBoldEnd] = md.match(poetryBoldOption) || [
        //   null,
        //   '__',
        //   '__',
        // ]
        ...
        renderer.heading = (text, level) => {
          ...
          return `<h${level}>${
            encodeURI(id) === id ? '' : `<span id="${encodeURI(id)}"></span>`
          }
        ...
        const processPoetry = rules => (match, $1, __, code) => {
          ...
          return langs
            ? langs
                .map(
                  lang =>
                    `<pre><code class="language-${lang}" poetry>${res}</code></pre>`,
                )
                .join('\n')
            : `<pre>${res}</pre>`
        }
        ...
        const processCodeGroup = (match, $1, $2) => {
          ...
          const [itemsContent, itemsTitles] = [
            ...items.matchAll(itemMarker),
          ].reduce(
            ...
            <div class="contentWrapper">
              ${itemsContent
                .map(
                  (item, index) =>
                    `<div class="content${
                      (code ? code === IDfy(itemsTitles[index]) : index === 0)
                        ? ' active'
                        : ''
    ...
    const html = `<div id="markdown-body" class="markdown-body${
      opts.classes
        ? this.arrify(opts.classes).reduce((a, c) => `${a} ${c}`, ' ')
        : ''
  ...
  async stampStyles(html) {
    ...
    const target = [...this.root.children].find(n =>
      n.classList.contains('markdown-styles'),
    )
    if (target) {
          ...
          (tabsWrapper.onclick = e => {
            ...
            if (isElementANonActiveTabButton) {
              if (this.config.groupCodeGroups) {
                ...
                node
                  .querySelectorAll(`.content,.inline-content`)
                  .forEach(content => {
                ...
                wrapper
                  .querySelector(`#${newActiveContentId}`)
...
document.addEventListener('set-value-to-editor', e => {
  // console.log('set-value-to-editor', e.detail)
  const lastZeroMdElement = e.detail.lastZeroMdElement
  const absolutePath = e.detail.absolutePath
  const flexGrowArray = e.detail.flexGrowArray
  ...
  // const zeromdClass = e.detail.zeromdClass
  // const idfyFucntion = e.detail.idfyFucntion

  // console.log('zeromdClass', zeromdClass)
  // console.log('idfyFucntion', idfyFucntion)

  // const convertedFunction = new Function(`return ${zeromdClass}`)()
  // console.log('convertedFunction', convertedFunction)

  setTimeout(async () => {
      // let contentScript = await fetch('https://cdn.jsdelivr.net/gh/ArtemPaskall/zero-md@9dee0b52a28b23f98b31562bf095dd8bd4793fac/dist/zero-md.min.js')
      // contentScript = await contentScript.text()
      // eval(contentScript)
      // console.log('contentScript', contentScript)


    // let contentScript = await fetch('https://cdn.jsdelivr.net/gh/ArtemPaskall/zero-md@213f4320e3233571154bcc3a52f8d4eab01cbf07/dist/zero-md.min.js')
    // let contentScript = await fetch('https://cdn.jsdelivr.net/gh/ArtemPaskall/zero-md@225da732074b00852ff1d955b3c6f35604716079/src/index.js')
    // contentScript = await contentScript.text()


    // console.log('contentScript'  , contentScript)
    // console.log(typeof contentScript)

    // eval(contentScript)

    const ZeroMDInstance = new ZeroMd()
    // const ZeroMDInstance = new convertedFunction()
    ZeroMDInstance.path = absolutePath

    const markDown = await ZeroMDInstance.buildMd()
    const css = ZeroMDInstance.buildStyles()

    const stampedBody = ZeroMDInstance.stampBody(markDown)
    ...
    const translationScript =
      ...
      'console.log("attributesWas Changed")\n' +
      ...
      'console.log("shgdfghdfghdfgh", markdownBody[0])\n' +
      ...
      'console.log("changeActiveStyleOnAttributeChange")\n' +
      ...
      'if (match.includes(code.toUpperCase()) || match.includes(lang.toUpperCase())) {\n' +
      'console.log("active")\n' +
    ...
    console.log('stampedBodyHtml', stampedBodyHtml)
